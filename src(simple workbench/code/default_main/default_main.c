...
static void convert_bbst_to_android(FILE *filep_in,FILE *filep_out);
...
static int default_main(void){
	...
	wchar_t *android_codec_dir = L"/android使用";
	...
	if (generate_object_directory(NULL,output_directory,android_codec_dir) == 1)return DIR_ERROR;
	...
	//android_codec_dir
	FILE *f_android = generate_object_file(NULL,output_directory,android_codec_dir,L"/android_codec.txt");
		if (f_android == NULL)return FILE_ERROR;
	...
	convert_bbst_to_android(f_bbst,f_android);
...
static void convert_bbst_to_android(FILE *filep_in,FILE *filep_out){
	fseek(filep_in,2,SEEK_SET);//2 to jump BOM
	fseek(filep_out,2,SEEK_SET);//2 to jump BOM
